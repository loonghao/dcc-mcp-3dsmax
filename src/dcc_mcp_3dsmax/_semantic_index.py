"""Optional morphology-aware semantic skill recall for 3ds Max.

``MaxMcpServer.search_skills`` routes through the Rust BM25-lite scorer in
...
``"modelling a chair"`` does not tokenise to the literal ``model`` token in a
``3dsmax-modeling`` skill's name / summary.

This module (a faithful port of the Maya implementation) fuses the Python-side
``VectorSkillIndex`` (``HashedEmbedder`` char-3-gram defaults, zero runtime
deps) with a ``LexicalSkillIndex`` through ``RrfFusionIndex`` (RRF k=60).  The
fused index is used **only to augment** the canonical base results: base
ordering is preserved verbatim and vector-only recalls are appended afterwards.

Gated behind ``DCC_MCP_3DSMAX_SEMANTIC_INDEX=1`` (default off).  When the
optional ``dcc-mcp-core[semantic]`` extra is installed,
``DCC_MCP_3DSMAX_SEMANTIC_EMBEDDER=onnx`` swaps ``HashedEmbedder`` for the real
...
def resolve_semantic_index_enabled(env: Optional[dict] = None) -> bool:
    """Return ``True`` when ``DCC_MCP_3DSMAX_SEMANTIC_INDEX`` is truthy."""
...
def _build_embedder(kind: str) -> Any:
    ...
    try:
        ...
        logger.warning(
            "[3dsmax] DCC_MCP_3DSMAX_SEMANTIC_EMBEDDER=onnx requested but unavailable "
...
class MaxSemanticIndex:
    ...
    # ── construction ────────────────────────────────────────────────────
    @classmethod
    def build(cls, *, embedder_kind: Optional[str] = None) -> Optional["MaxSemanticIndex"]:
        ...
        except Exception as exc:  # noqa: BLE001 — older core
            logger.info(
                "[3dsmax] semantic index requested but dcc-mcp-core lacks "
        ...
        try:
            ...
            logger.warning("[3dsmax] failed to build semantic fusion index: %s", exc)
            return None
    ...
    def recall(self, query: str, *, k: int = 16) -> List[str]:
        ...
        try:
            ...
            logger.debug("[3dsmax] semantic recall failed for %r: %s", query, exc)
            return []
    ...
    def augment(
        ...
        *,
        ...
        try:
            ...
            logger.debug("[3dsmax] semantic rebuild failed: %s", exc)
            return result
