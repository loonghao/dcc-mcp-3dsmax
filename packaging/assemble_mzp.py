...
The output is a ZIP-compatible ``.mzp`` archive with ``mzp.run`` at the
archive root.  3ds Max executes ``mzp.run`` when users drag the archive into
the viewport.
"""
...
def resolve_core_version(project_root: Path) -> str:
    ...
    match = re.search(r"dcc-mcp-core>=(\d+\.\d+\.\d+)", content)
    if not match:
        raise RuntimeError("Cannot find dcc-mcp-core minimum version in pyproject.toml")
    minimum = match.group(1)

    try:
        with urllib.request.urlopen("https://pypi.org/pypi/dcc-mcp-core/json", timeout=15) as resp:
            data = json.loads(resp.read())
        latest = data.get("info", {}).get("version", "")
        if latest and _version_gte(latest, minimum):
            print(f"Resolved dcc-mcp-core {latest} from PyPI")
            return latest
    except Exception as exc:  # noqa: BLE001
        print(f"Warning: could not query latest dcc-mcp-core ({exc}); using {minimum}")
...
-- Adds the installed Python packages to sys.path when 3ds Max starts.
(
    local installRoot = getDir #userScripts + "\\\\dcc_mcp_3dsmax"
    local py = ""
...
(
    ...
    local userStartupScripts = getDir #userStartupScripts
    local py = ""
    ...
    python.Execute py
    messageBox "dcc-mcp-3dsmax installed. Restart 3ds Max, then import dcc_mcp_3dsmax from Python." title:"dcc-mcp-3dsmax"
)
""",
        encoding="utf-8",
    )
