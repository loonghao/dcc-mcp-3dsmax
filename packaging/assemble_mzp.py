...
(
    ...
    local uninstallMarker = _dccMcpNormalizePath (getDir #userScripts + "/dcc_mcp_3dsmax/dcc_mcp_3dsmax_uninstall_pending")
    ...
    py += "            return candidate\\n"
    py += "    return root\\n"
    py += "if uninstall_marker.exists():\\n"
    py += "    if root.exists():\\n"
    py += "        shutil.rmtree(root)\\n"
...
(
    ...
    (
        ...
        py += "uninstall_marker = install_dir / 'dcc_mcp_3dsmax_uninstall_pending'\\n"
        ...
        py += "_install_menu(version_dir)\\n"
        py += "print('Installed dcc-mcp-3dsmax', version_name, 'to', version_dir)\\n"
        try
        (
            python.Execute py
            messageBox "dcc-mcp-3dsmax installed as an isolated version. Restart 3ds Max before starting the sidecar if an older version was already loaded." title:"dcc-mcp-3dsmax"
        )
        catch
        (
